...
import { WithWithoutTab } from "@/components/profile/tabs/WithWithoutTab";
import {
  RADAR_IN_POSSESSION, RADAR_TRANSITIONS, RADAR_OUT_POSSESSION, STANDINGS,
} from "@/data/enzo-millot-extended";
import type { Standing } from "@/data/enzo-millot-extended";

type SubTab = "style" | "withwithout" | "classement";
...
function FitScoreWidget() {
  ...
  return (
    ...
    >
      <span className="text-xs font-semibold uppercase tracking-wider mb-4 text-center" style={{ color: "var(--color-neutral-400)", letterSpacing: "0.06em" }}>
        Fit Score &amp; Similarity
      </span>
      <svg width={130} height={130}>
        <circle cx={65} cy={65} r={R} fill="none" stroke="var(--color-neutral-700)" strokeWidth={8} />
        <circle
          cx={65} cy={65} r={R} fill="none" stroke="#C42B47" strokeWidth={8}
        ...
        <text x={65} y={77} textAnchor="middle" style={{ fill: "var(--color-neutral-500)", fontSize: 11 }}>/100</text>
      </svg>
...
// ── Style de jeu sub-tab ──────────────────────────────────────────────────────
function StyleDeJeuSubTab() {
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center gap-2">
        <span
          ...
          style={{ backgroundColor: "rgba(196,43,71,0.10)", border: "1px solid rgba(196,43,71,0.25)", color: "var(--color-primary-300)" }}
        >
          FC Metz — Métriques d&apos;Équipe
        </span>
        <span className="text-xs" style={{ color: "var(--color-neutral-600)" }}>
          Données collectives · Ligue 1 2024-25
        </span>
      </div>

      {/* 3 sectored radars + Fit Score */}
